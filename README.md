...
| `poll_incremental` | ~2 min | reflect **every page changed since the watermark** (deep sub-pages incl.) via Notion `/search`; also regenerates `_Dashboard`/`_Commands` + drops deleted/archived **spine** items |
| `full_reconcile` | daily, 04:00 (cron) | backstop only: **deep-page** deletions, orphan pruning, drift healing, regenerate Docs |
...
| Manual **delete/archive** of a spine item (area/project/action) | **~2 min** | `poll_incremental` spine sweep |
| Manual delete of a **deep sub-page**, new deep subtree, orphan cleanup | **next 04:00** | daily `full_reconcile` backstop (search can't report deletions) |
